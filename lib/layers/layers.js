 ...
 * @param routes The routes object. See Layers#setupRoutes for more information.
...
var Layers = function(server, rootPath, routes, options) {
    ...
    this.setupRoutes(server, routes(server));
};

/**
 * Creates routes on a server based on the routes definition 
 * object supplied.
 * 
 * @param server The Server to add the routes to.
 * @param routes The Routes object.
 *
 * The routes object is a map of arrays of route objects.
 * i.e.
 * 
 * {
 *   "/": [{
 ...
 *
 */
Layers.prototype.setupRoutes = function(server, routes) {
    var self = this;
        ...
        files.forEach(function(fileName) {
            ...
            if (isDirectory(fullPathToFile)) {
                ...
                var name = getFileName(fileName),
                    clazz = require(path + "/" + name);

                if (fileName.indexOf(options.excludePrefix) == -1) {
                    var name = getFileName(fileName),
                        clazz = require(path + "/" + name);

                    server[layer][getInstanceName(name)] = new clazz(server);
                }
            }
        });
    }
